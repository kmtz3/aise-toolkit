...
### Fixed
- **`skills/session-facilitation/SKILL.md` — the guide never reached Drive and never got linked into Planhat.** The artifact convention landed in 2.51.0 but only into the orchestrators (`session-prepper` § 6.8, `daily-brief` § 5.5, `bulk-prep-week` § 5.5); the skill itself still wrote to `~/Desktop/aise-assistant/facilitation/` and appended a **Notion** file-path callout, so any standalone `/session-facilitation` run produced a file that existed on one laptop and nowhere else. Confirmed 2026-09-01: the `Customer Session Artifacts` folder held exactly one file (`Emplifi_2026-08-27_…_SessionPrep.html`) and CFC Underwriting's 1 Sep A1 session had prep notes with no artifact block. Step 3 is now the local **working copy** (best-effort, non-blocking), new step 4 is a mandatory Drive publish + `custom.Prep Notes` link-back on the GCal-synced event Task (Conversation fallback) with read-back verification, and the Notion callout is retired with an explicit do-not-re-add note. File naming moves from the old `{customer-slug}-{session-id}-facilitation-{date}.html` to the convention's `{CustomerName}_{YYYY-MM-DD}_{SalesforceAccountId}_Facilitation.html`, with the Salesforce Account Id resolved in step 1 instead of mid-publish.
- **`skills/session-facilitation/SKILL.md` + `agents/session-prepper.md` § 6.8 — double-publish guard.** Now that the skill publishes itself, both paths state the same idempotency rule: same filename, update in place, one link block, and report the skip.
- **Docs swept to match:** `CLAUDE.md` § command table, `skills/assistant-help/SKILL.md` § session workflow, and `agents/session-prepper.md` § 7a no longer advertise a Desktop path or a Notion link for the facilitation guide.
- **`agents/post-session-debrief.md` § 3 — the Conversation `date` row hardcoded `T00:00:00.000Z`, destroying every session's time.** The payload table instructed "Session date as ISO 8601 (`T00:00:00.000Z`)", so every debrief stamped its Conversation at midnight. Because the resolution ladder resolves to the *existing* Planhat-created Conversation and updates it, this actively overwrote a real timestamp with a fake one. Found 2026-08-29 on Emplifi `6a8d3837e1a0f3e92ccf461c` — Conversation `date` `2026-08-27T00:00:00.000Z` against a coupled Task `startTime` of `08:30:00Z`. The row now resolves the real start through the § Session timestamp ladder, corrects a wrong existing value on update, and reports the before/after; `startDate` is split out as day-only, which is what the field's `date` type actually holds.
- **`agents/post-session-debrief.md` § 3 — the Conversation `description` row carried no format rule, so session notes shipped as flat text.** Every *other* description write in the file (Task bodies at §§ 4, 6, 8; `custom.Next Step` at § 10) specifies single-line `ph-editor` HTML, and `CLAUDE.md` § Planhat rich-text fields names `Conversation description` explicitly — but the step 3 payload row said only "truncate to ~2000 chars", and in practice the local row wins. Literal `\n` is stripped by the API on write, so the field landed as one unbroken run. Confirmed on the same Emplifi record. Rule added inline with the section-label and list shapes.
- **`agents/ph-migrate-notion-data.md` § 5 — same two defects on the migration path.** `date` now runs the timestamp ladder (the Task resolved in § 4 already carries `startTime`) and falls back to Notion's date-only `Call Date` **only** as a last resort, counted under a `⚠️ DATE TIME UNRESOLVED` line so midnight stamps stay visible instead of passing as real times. `description` now requires conversion from Notion markdown into single-line `ph-editor` HTML rather than a raw paste.
- **`agents/session-log-auditor.md` § 9.3 — duplicate merges appended `description` as a raw concatenation.** Now specified as single-line HTML with an `<hr>` provenance header.
- **`skills/ph-reconcile-gong-gcal/SKILL.md`, `agents/ph-reconcile-gong-gcal.md` — `--window-hours` default raised from 4 to 12.** A midnight-stamped target sits up to ~12h from its own Gong call, so a ±4h window structurally could not find it: the 2026-08-29 Emplifi dry run scored zero candidates at the default and 0.823 (attendee 1.00 · subject 1.00) at 12. Date proximity is the weakest signal at 0.25 and attendee overlap is exact-ID, so the widening costs little precision.
- **`agents/ph-reconcile-gong-gcal.md` § 4 — read the source Gong URL from `custom.Call Recording` first.** Since the 2026-08-27 field migration Gong's own sync writes the call link to `custom.Call Recording` and leaves `custom.Gong URL` empty (verified on Emplifi `6a9006bc8ab9b10391dc6508`, where the documented source field was blank). `custom.Gong URL` stays as a fallback for pre-migration records. Also: a target already holding the *identical* recording URL is no longer treated as a conflict that blocks the whole merge — the field is skipped and the merge proceeds.

### Added
- **`context/planhat-schema.md` § Session timestamp — new canonical section.** Documents that Planhat stamps a converted calendar-event Conversation's `date` with **the moment the Task was marked done**, not the session start, with two verified pairs showing the drift is arbitrary: IBS Software `6a8c5defe1739d6cb5c88886` +74 min, Validity `6a75d97d97ccd97bc0fcd795` +2 days and onto the wrong calendar day. Millisecond precision on `date` is the tell. Defines the resolution ladder — coupled Task `startTime` → GCal event start → Gong call time → leave untouched and report — the correct-on-every-touch rule, the reporting line, and the types it does **not** apply to (`💬 Slack Chat`, `email`/`chat`/`ticket`, `👾 Gong Call`).
- **`context/planhat-schema.md` § Session record resolution — the Task and its Conversation share an `_id` but not their fields.** Explains why a Task prepped before its conversion keeps a stale `custom.Prep Notes` forever (expected, not a bug — the Conversation is canonical), and why that coupling makes the timestamp fix a single `get_model_record` call with no `sourceId` lookup.
- **`CLAUDE.md` § Ground rules — "Correct the session timestamp on every session Conversation you touch."** Applies the rule to any agent writing a session record, not only the debrief path.
- **`agents/session-log-auditor.md` § 6f + § 9.4, `skills/session-audit/SKILL.md` — misdated sessions are now an audited defect class with a `--dates` flag.** The audit already pulls the calendar and Gong, so it is the natural repair pass for historical records. Splits **time-only drift** (safe to bulk-correct) from **day drift** (never auto-written — a wrong day usually means the record is matched to the wrong event). Ordered after duplicate merges, since correcting a midnight stamp can turn a "midnight/clock-time pair" into an exact duplicate.
- **`agents/session-prepper.md` § 5b-2 — the Conversation-hit branch now corrects `date` in the same update** it was already making for `custom.Prep Notes`, and documents the stale-Task-copy behavior inline.

### Known inconsistency
- `skills/session-facilitation/SKILL.md` step 1 still sources session identity, open items, outcomes and pre-reads from **Notion** (Sessions DB, `📋 Prep` toggle) while prep notes are now written to Planhat. Left as-is deliberately: the read-side Notion→Planhat migration is a separate sweep across every session skill, and changing it here alone would split the flow. Only the write side (artifact storage + link-back) moved in this release.
