...
- `--apply` (optional) — actually write merges and delete Gong Call records. **Default is dry-run: preview only, no writes, no deletes.** Even with `--apply`, still present the plan and get one explicit confirmation before executing — `--apply` skips nothing, it only means "you don't have to ask me to run the live pass after the dry-run looks right."
