...
1. **Resolve identity** (needed for owner-scoped queries):
   1. `notion-get-users` (self) → `notion_user_id`, display name, email. No Planhat lookup required — the Notion UUID used for ownership scoping isn't part of the Planhat profile (see `context/planhat-user-profile.md`).
