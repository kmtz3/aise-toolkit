...
4. Both of these return Planhat `_id`s directly — no separate identity lookup is needed. Use the resolved `_id` for any team-scoped Planhat query (`ownerId` on Tasks, `users` on Conversations, Company `owner`).
...
**Applies to:** any agent that would otherwise hard-stop on a missing `custom.AISE Identity` (or other core field) — `daily-brief`, `session-log-auditor`, `bulk-account-setup`, `session-backfill`, `bulk-prep-week`, `aise-context`, `log-feedback`, and the aise-leadership equivalents (`report-builder`, `session-log-auditor`, etc.).
